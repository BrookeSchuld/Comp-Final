...
for i in range(len(Pion_KE)):
    if len(Pion_Dedx[i]>3): Pion_Dep.Fill(Pion_KE[i],np.mean(Pion_Dedx[i][-3:-1]))
Pion_Dep.Draw("colz")
...
for i in range(len(Proton_KE)):
    if len(Proton_Dedx[i])>3: Proton_Dep.Fill(Proton_KE[i],np.mean(Proton_Dedx[i][-3:-1]))
...
for i in range(len(Kaon_KE))
    if len(Kaon_Dedx[i])>3: Kaon_Dep.Fill(Kaon_KE[i],np.mean(Kaon_Dedx[i][-3:-1]))
Kaon_Dep.Draw("colz")
